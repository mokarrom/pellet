...
public class NotAllValuesRule extends AbstractTableauRule {
	...
	public void apply( Individual x ) {
        if( !x.canApply( Individual.NOTALL ) )
        	return;

        List<ATermAppl> types = x.getTypes( Node.NOTALL );
        int size = types.size();
        for( int j = x.applyNext[Node.NOTALL]; j < size; j++ ) {
            ATermAppl nav = types.get( j );

            applyNotAllValuesRule( x, nav );
            
            if( strategy.getABox().isClosed() || x.isPruned() )
                return;
        }
        x.applyNext[Individual.NOTALL] = size;
    }
    ...
    protected void applyNotAllValuesRule( Individual x, ATermAppl nav ) {
        ...
        DependencySet ds = x.getDepends( nav );

        
        if(!PelletOptions.MAINTAIN_COMPLETION_QUEUE && ds == null)
			return;
        
        c = ATermUtils.qcNegate( c );	//since qcnot(c) is added to the new node.
