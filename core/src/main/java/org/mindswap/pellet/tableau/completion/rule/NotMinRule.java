...
public class NotMinRule extends AbstractTableauRule {
	public NotMinRule(CompletionStrategy strategy) {
		super( strategy, NodeSelector.NOT_MIN_NUMBER, BlockingType.INDIRECT );
	}
	
	/**
     * Apply notmin rule to the individual.
     */
    public void apply( Individual x ) {
        if( !x.canApply( Individual.NOTMIN ) )
        	return;

        List<ATermAppl> types = x.getTypes( Node.NOTMIN );
        int size = types.size();
        for( int i = 0; i < size; i++ ) {
            ATermAppl mc = types.get( i );

            applyNotMinRule( x, mc );
        ...
        x.applyNext[Individual.NOTMIN] = size;
    }
    
    protected void applyNotMinRule( Individual x, ATermAppl mc ) {
    	 
    	// notmin(r, n) is in normalized form qcnot(min(r, n))
        ATermAppl max = (ATermAppl) mc.getArgument( 0 );	//now, max = min(r, n)

        Role r = strategy.getABox().getRole( max.getArgument( 0 ) );
        int n = ((ATermInt) max.getArgument( 1 )).getInt();
        ...
        if( n == 1 ) {
        	applyFunctionalNotMinRule( x, r, c, ds );
            if( strategy.getABox().isClosed() )
                return;
        }
        else {
            ...
            while( hasMore ) {
            		hasMore = applyNotMinRule( x, r, c, n, ds );
     ...
     */
    protected boolean applyNotMinRule( Individual x, Role r, ATermAppl c, int k, DependencySet ds ) {
    ...
    public void applyFunctionalNotMinRule( Individual x, Role s, ATermAppl c, DependencySet ds ) {
